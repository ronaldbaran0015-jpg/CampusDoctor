    ...
    <div class="chatbot-popup">
        ...
        <div class="chat-header">
            <div class="header-info">
                @if (!$sender->image)
                <img src="{{asset('assets/svg/man-svgrepo-com.svg')}}" alt="patient-logo" class="chatbot-logo">
                @else
                <img src="{{asset('uploads/patients/'. $sender->image)}}" alt="patient-logo" class="chatbot-logo">
                @endif
                <h2 class="logo-text">{{$sender->name}}</h2>
            </div>
            <div class="px-2">
        ...
        <div class="chat-body">
            ...
            @if($message->sender_id == Auth::guard('doctor')->user()->docid && $message->sender_type =='doctor' )
            <div class="message user-message position-relative px-3">
                ...
                <p class="message-text m-0" data-message-id="{{ $message->id }}" style="width:auto; max-width: 75%;">{{ $message->message }}</p>
            </div>
            @else
            <div class="message bot-message">
                @if (!$sender->image)
                <img src="{{asset('assets/svg/man-svgrepo-com.svg')}}" alt="patient-avatar" class="bot-avatar">
                @else
                <img src="{{asset('uploads/patients/'. $sender->image)}}" alt="patient-avatar" class="bot-avatar">
                @endif

                <!-- prettier-ignore -->
                <div class="message message-text" style="width:auto; max-width: 75%;"> {{ $message->message }} </div>
            </div>
            <!-- <div class="message bot-message">{{ $message->message }}</div> -->
            @endif
            @endforeach
        </div>
        <!-- Chatbot Footer -->
        <div class="chat-footer">
            <form action="" method="post" class="chat-form">
                @csrf
                <input type="hidden" name="receiver_id" value="{{ $sender->pid }}">
            ...
            $('#send-message').on('click', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#message-input').val('');
                        var html = '<div class="message user-message"><p class="message-text" style="width:auto; max-width: 75%;">' + message + '</p></div>';
